...
MODEL_NAMES = ("cnxb384_full.pth", "dinov2b_full.pth", "fnoise_full.pth")
...
def rank_norm(values: np.ndarray) -> np.ndarray:
    """Ordinal ranks in [0, 1], matching the frozen training-repo inference."""
...
def main() -> int:
    ...
    print(
        ...
        f"batch_size={args.batch_size} workers={args.workers} tta={not args.no_tta}",
        file=sys.stderr,
    )
    ranked = []
    for checkpoint_path in checkpoint_paths:
        scores = predict_checkpoint(
            ...
            device, tta=not args.no_tta,
        )
        ranked.append(rank_norm(scores))

    labels = np.mean(np.stack(ranked), axis=0)
